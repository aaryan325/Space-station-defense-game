...
class CharacterPen():
    ...
    def draw_character(self, pen, character, x, y):
        ...
        #Check for characters is in the dictionary
...
character_pen.scale = 1.0
character_pen.draw_string(pen, "BY AARYANSARDA", 0, 100)
...
class Enemy(Sprite, Game):
    def __init__(self, x, y, shape, color):
        Sprite.__init__(self, x, y, "square", "red")
        self.max_health = 20
        self.health = self.max_health
...
class Radar():
    ...
    def render(self, pen, sprites):
        ...
        #Draw Radar circle
        pen.setheading(90)
        pen.goto(self.x + self.width / 2.0, self.y - self.width - 400)
        pen.pendown()
        ...
        for sprite in sprites:
            if sprite.state == "active":
                ...
                radar_y = self.y + (sprite.y - player.y - 1475) * (self.height / game.height)
...
player = Player(0, 0, "player", "white")

camera = Camera(player.x, player.y)

#Create object of missile class
missiles = []
for _ in range(3):
